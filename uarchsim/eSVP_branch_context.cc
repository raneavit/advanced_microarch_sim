...
eSVP_branch_context::eSVP_branch_context(uint64_t vpq_size_in, uint64_t prediction_mode_in, uint64_t confidence_mode_in, uint64_t indexBits_in, 
        ...
        bool pred_fpu_alu_in, bool pred_load_in, uint64_t vpq_full_policy_in, uint16_t max_age_in, uint32_t bhr_bits_in, payload *PAY_in, pipeline_t *proc_in, renamer* REN_in, BPinterface_t *CBP_in)
{
...
void eSVP_branch_context::predict(uint64_t index){
...
void eSVP_branch_context::perf_val_pred(uint64_t index){
    db_t* actual;
...
void eSVP_branch_context::real_val_pred(uint64_t index){
    db_t* actual;
    ...
    temp.pc_tag = getTag(pc);
    bhr = PAY->buf[index].bhr_checkpoint;
    temp.pc_index = getIndex(pc);
...
void eSVP_branch_context::inject_val_dispatch(uint64_t index){
    REN->write(PAY->buf[index].C_phys_reg, PAY->buf[index].predicted_value);
    REN->set_ready(PAY->buf[index].C_phys_reg);
}


uint64_t eSVP_branch_context::getTag(uint64_t pc){
...
uint64_t eSVP_branch_context::getIndex(uint64_t pc){
    uint64_t bitmask, temp, temp_bhr;
    temp_bhr = bhr >> (64-bhr_bits);
    temp = (pc >> 2);
    bitmask = 1;
    bitmask = bitmask << indexBits;
    bitmask = bitmask  - 1;
    temp = (temp & bitmask);

    return (temp ^ temp_bhr);
}

void eSVP_branch_context::set_val_in_vpq(uint64_t vpq_index, uint64_t val){

    if(prediction_mode == REAL_VAL_PRED) vpq->fifoList[vpq_index].value = val;
 
}

void eSVP_branch_context::train_at_retire(){
    uint64_t svp_index;
    vpq_entry temp;
    int64_t new_stride;
    uint64_t pred_val;
    ...
    temp = vpq->pop();
    svp_index = temp.pc_index;

   
    if(((tagBits>0) && (temp.pc_tag == svp_table[svp_index].tag)) || (tagBits == 0)){
        ...
        if(svp_table[svp_index].conf->getCount() <=replace_entry){
            svp_table[svp_index].tag = temp.pc_tag;
            svp_table[svp_index].conf->setCount(0);
            svp_table[svp_index].retired_val = temp.value;
            svp_table[svp_index].stride = temp.value;
            svp_table[svp_index].instance = find_instances(temp.pc_tag, temp.pc_index);//walk vpq
...
uint64_t eSVP_branch_context::find_instances(uint64_t tag, uint64_t index){
    uint64_t search_index;
    uint64_t instances = 0;
    search_index = vpq->head;

    do { //This works with the assumption that VPQ isnt empty
        if((vpq->fifoList[search_index].pc_tag == tag) && (vpq->fifoList[search_index].pc_index == index)) instances++;
...
void eSVP_branch_context::checkpoint(uint64_t& chkpt_vpq_tail, bool& chkpt_vpq_tail_phase){
    chkpt_vpq_tail = vpq->tail;
    chkpt_vpq_tail_phase = vpq->tailPhase;
}


void eSVP_branch_context::restore(uint64_t recover_vpq_tail, bool recover_vpq_tail_phase){
    uint64_t svp_index;
    while((vpq->tail != recover_vpq_tail) || (vpq->tailPhase != recover_vpq_tail_phase)){
        ...
        svp_index = vpq->fifoList[vpq->tail].pc_index;

        if(((tagBits>0) && (vpq->fifoList[vpq->tail].pc_tag == svp_table[svp_index].tag)) || (tagBits == 0)) svp_table[svp_index].instance--;

    }
}

bool eSVP_branch_context::stall_prediction(uint64_t bundle_size){
    uint64_t free_space;
...
void eSVP_branch_context::squash_vpq(){
    vpq->tail = vpq->head;
...
void eSVP_branch_context::dump_stats(FILE* fp) {
	int ins_total;
...
bool eSVP_branch_context::eligible(payload_t *pay) {
...
void eSVP_branch_context::configure_probabilistic_counters(){
    // confmax 7 
    increment_data[LOAD] = {0.10, 1};
    increment_data[SIMPLE_INT_ALU] = {0.08, 1};
    ...
    // confmax 3 
    // increment_data[LOAD] = {0.02, 1};
    // increment_data[SIMPLE_INT_ALU] = {0.01, 1};
    // increment_data[COMPLEX_INT_ALU] = {0.02, 1};
    // increment_data[FP_ALU] = {0.02, 1};
    // increment_data[AGE] = {0.002, 1};
    // increment_data[REDUCE_CONF] = {0,0};
    // increment_data[DEFAULT] = {0.02, 1};
...
void eSVP_branch_context::print_info(FILE* fp){
    uint64_t confBits, instanceBits, ageBits, svp_entry_bits, vpq_entry_bits, totalBits, svp_entries, prob_counter_overheads, bhr_overhead;
    ...
    fprintf(fp, "\t\t -------------------------\n");
    svp_entry_bits = SVP_TAG_BITS + confBits + 64 + 32 + instanceBits + ageBits;
    ...
    fprintf(fp, "\t BHR Overheads:\n");
    fprintf(fp, "\t\t (# BHR bits) * [ 8 (for fetch2 pipe register) + 8 (for decode pipe register) + 64 (for FQ) + 8 (for rename2 pipe register) + VPQ_size]\n");
    bhr_overhead = bhr_bits*(8+8+64+8+SVP_VPQ_SIZE);
    fprintf(fp, "\t\t BHR overhead       :    %lu bhr bits x (8 + 8 + 64 + 8 + %lu) =  %lu \n", bhr_bits, SVP_VPQ_SIZE, bhr_overhead);
    fprintf(fp, "\t\t -------------------------\n");
    

    totalBits = svp_entries*svp_entry_bits + SVP_VPQ_SIZE*vpq_entry_bits + prob_counter_overheads + bhr_overhead;
    fprintf(fp, "\t Total storage cost (bits) = %lu (%lu SVP entries x %lu bits/SVP entry) + %lu (%lu VPQ entries x %lu bits/VPQ entry) + %lu (Probabilistic Counter Overhead) + %lu(BHR Overheads) = %lu bits\n",
                                    svp_entries*svp_entry_bits, svp_entries, svp_entry_bits,
                                    SVP_VPQ_SIZE*vpq_entry_bits, SVP_VPQ_SIZE, vpq_entry_bits, prob_counter_overheads, bhr_overhead, totalBits);
