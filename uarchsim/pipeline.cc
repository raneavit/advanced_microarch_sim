  ...
  LSU(lq_size, sq_size, Tid, _mmu, this)
{
  ...
  if(VP_TYPE == 0){
    VPU = new stride_val_pred(SVP_VPQ_SIZE, VP_PREDICTION_MODE, SVP_CONFIDENCE_MODE, SVP_INDEX_BITS, 
                            SVP_TAG_BITS, SVP_CONFMAX, SVP_CONFINC, SVP_CONFDEC,
                            SVP_REPLACE_STRIDE, SVP_REPLACE, SVP_PRED_INT_ALU, 
                            SVP_PRED_FP_ALU, SVP_PRED_LOAD, SVP_VPQ_FULL_POLICY,  &PAY, this, REN);

  }
  else if(VP_TYPE == 1){
    VPU = new eSVP(SVP_VPQ_SIZE, VP_PREDICTION_MODE, SVP_CONFIDENCE_MODE, SVP_INDEX_BITS, 
                          SVP_TAG_BITS, SVP_CONFMAX, SVP_CONFINC, SVP_CONFDEC,
                          SVP_REPLACE_STRIDE, SVP_REPLACE, SVP_PRED_INT_ALU, 
                          SVP_PRED_FP_ALU, SVP_PRED_LOAD, SVP_VPQ_FULL_POLICY, ESVP_MAX_AGE,  &PAY, this, REN);
  }
  else if(VP_TYPE == 2){
    VPU = new eSVP_branch_context(SVP_VPQ_SIZE, VP_PREDICTION_MODE, SVP_CONFIDENCE_MODE, SVP_INDEX_BITS, 
        SVP_TAG_BITS, SVP_CONFMAX, SVP_CONFINC, SVP_CONFDEC, 
        SVP_REPLACE_STRIDE, SVP_REPLACE, SVP_PRED_INT_ALU, 
        SVP_PRED_FP_ALU, SVP_PRED_LOAD, SVP_VPQ_FULL_POLICY, ESVP_MAX_AGE, ESVP_BHR_BITS, &PAY, this, REN, FetchUnit->getCBP());
  }

  value_prediction_flag = VP_ENABLE_FLAG;
