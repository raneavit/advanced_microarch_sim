...
void pipeline_t::dispatch() {
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      // FIX_ME #8 BEGIN
      if(PAY.buf[index].A_valid) A_ready = REN->is_ready(PAY.buf[index].A_phys_reg);
      else A_ready = true;

      if(PAY.buf[index].B_valid) B_ready = REN->is_ready(PAY.buf[index].B_phys_reg);
      else B_ready = true;

      if(PAY.buf[index].D_valid) D_ready = REN->is_ready(PAY.buf[index].D_phys_reg);
      else D_ready = true;
      ...
      if(PAY.buf[index].C_valid){
         if(!PAY.buf[index].is_value_predicted) REN->clear_ready(PAY.buf[index].C_phys_reg);
         else{
            VPU->inject_val_dispatch(index);
         }
      }
      // FIX_ME #9 END
