   ...
   if (Execution_Lanes[lane_number].ex[depth].valid) {
      ...
      if (IS_MEM_OP(PAY.buf[index].flags)) {
	    ...
	    // check the existence (validity) of a destination register.
            ...
            //Handling Value prediction for loads
            if(PAY.buf[index].is_value_predicted){
                  VPU->set_val_in_vpq(PAY.buf[index].vpq_index, PAY.buf[index].C_value.dw);
                  if(REN->read(PAY.buf[index].C_phys_reg) != PAY.buf[index].C_value.dw) 
                  {
               ...
               if(PAY.buf[index].in_vpq) VPU->set_val_in_vpq(PAY.buf[index].vpq_index, PAY.buf[index].C_value.dw);
               REN->write(PAY.buf[index].C_phys_reg, PAY.buf[index].C_value.dw);
