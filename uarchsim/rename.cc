...
void pipeline_t::rename2() {
   unsigned int i;
   unsigned int index;
   unsigned int bundle_dst, bundle_branch, bundle_predict;
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      if(PAY.buf[index].C_valid && value_prediction_flag && !PAY.buf[index].checkpoint){
            if(VP_PREDICTION_MODE == true) PAY.buf[index].vp_eligible = true; // perfect value prediction
            else{
               if((IS_INTALU(PAY.buf[index].flags)) && (VPU->pred_int_alu)) {PAY.buf[index].vp_eligible = true; bundle_predict++;}
               else if((IS_FPALU(PAY.buf[index].flags)) && (VPU->pred_fpu_alu)) {PAY.buf[index].vp_eligible = true; bundle_predict++;}
               else if((IS_LOAD(PAY.buf[index].flags) && !IS_AMO(PAY.buf[index].flags)) && (VPU->pred_load)) {PAY.buf[index].vp_eligible = true; bundle_predict++;}
               else PAY.buf[index].vp_eligible = false;
            }
      }
      else PAY.buf[index].vp_eligible = false;
      // FIX_ME #1 END
   ...
   // FIX_ME #2 BEGIN
   if(REN->stall_reg(bundle_dst) == true) return;
   if(REN->stall_branch(bundle_branch) == true) return;
   if(value_prediction_flag)
      if(VPU->stall_prediction(bundle_predict) == true) return;
   // FIX_ME #2 END
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      if(PAY.buf[index].checkpoint){
         ...
         if(value_prediction_flag){
            VPU->checkpoint(PAY.buf[index].vpq_index, PAY.buf[index].vpq_tail_phase);
         }
         }
      // FIX_ME #5 END
      ...
      //Value Prediction
      if(PAY.buf[index].vp_eligible) VPU->predict(index);
      else {
